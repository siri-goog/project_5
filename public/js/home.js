...
function searchMovies() {
  ...
  var searchGenre = $('body').find('#searchGenre').val()

  if (searchTitle !== "" || searchGenre.val !== 0) {
    var movieDetails = $('body').find('.column')
    movieDetails.hide()

    if (searchTitle !== "") {
      const endpoint = 'https://api.themoviedb.org/3/search/movie?query='+ searchTitle +'&api_key=fccb47b5258e28ee874723f58bb8cdc1'
      $.getJSON(endpoint)
        .then(result => {
          const movieRow = $('body').find('.row')
          for(var i = 0; i < result.results.length; i++) {
    ...
    if (searchGenre.val !== 0) {
      console.log("goog")
      const endpoint = 'https://api.themoviedb.org/3/discover/movie?api_key=fccb47b5258e28ee874723f58bb8cdc1&with_genres=' + searchGenre
      $.getJSON(endpoint)
        .then(result => {
          console.log(result)
          const movieRow = $('body').find('.row')
